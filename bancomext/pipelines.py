...
class BbancomextPipeline:

    # Database setup
    conn = sqlite3.connect('bancomext.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bancomext`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bancomext WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bancomext (date, title, link, content)"
